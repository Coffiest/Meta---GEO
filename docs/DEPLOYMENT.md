...
| `SUPABASE_URL` | SupabaseプロジェクトURL(Project Settings → API → Project URL。`DATABASE_URL`とは別物) |
| `SUPABASE_SERVICE_ROLE_KEY` | Supabaseのservice roleキー(Project Settings → API → `service_role` secret) |
| `NEXT_PUBLIC_SUPABASE_URL` | 上と同じProject URL(ブラウザに公開される値だが別名で登録する) |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Supabaseのanonキー(Project Settings → API → `anon` `public`) |
...
`SUPABASE_URL`以下の4つはログイン機能(メールリンク/Google/Apple)用です。**未設定でもデプロイは
失敗せず**、その場合はログイン機能自体が無効化され、従来の「表示名を入力して参加」というゲスト
フローにフォールバックします。ログイン機能を有効にしたい場合のみ登録してください。あわせて
Supabaseダッシュボードの `Authentication → Providers` で Email / Google / Apple を有効化する
必要があります。
...
| `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` | `packages/server` | ログイン時のアクセストークン検証用(GitHubシークレット) |
| `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY` | `apps/web` | ブラウザ側のログインUI用(GitHubシークレット) |

## 本番URLについて

Vercelはデプロイのたびに一意なURLを新規発行するため、そのままだとブックマークしたURLが毎回
無効になってしまいます。これを避けるため、`deploy-web`ジョブの最後で
`vercel alias set <デプロイURL> meta-geo-poker.vercel.app` を実行し、常に
**`https://meta-geo-poker.vercel.app`** を指すよう固定しています。今後はこのURLをブックマーク
してください(Fly.io側の `https://meta---geo.fly.dev` は元々固定URLです)。
