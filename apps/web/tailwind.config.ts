  ...
  theme: {
    extend: {
      colors: {
        ...
        /* ── プレイ画面(卓)専用の旧パレット ─────────────────────────
           卓画面はダークテーマ刷新の対象から外し、刷新前の白基調のまま据え置いている
           (オーナー指示)。この ink / gold はその卓画面だけが使う。
           **他の画面では絶対に使わない** ― アプリの他の全画面は canvas/surface/line/fg/accent
           の側で描かれており、ここを混ぜると2つの体系が同じ画面に同居してしまう。
           使用箇所は app/page.tsx・PokerTable.tsx・Seat.tsx・ActionBar.tsx の4つだけで、
           lint:tokens もその4ファイルだけを検査対象から外している。 */
        ink: {
          950: "#0a0a0a",
          900: "#171717",
          850: "#262626",
        ...
        gold: { 400: "#f7c548", 500: "#f2a900", 600: "#d4910a" },
