...
export function HandDetailPanel({ hand }: { hand: HandDetail | null }) {
  ...
  return (
    <div className="rounded-2xl bg-ink-900/70 ring-1 ring-ink-700/50 p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-gold-400 text-sm font-medium tabular-nums">POT {hand.potTotal.toLocaleString()}</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        {hand.seats.map((s) => (
          <div key={s.seatIndex} className="rounded-xl bg-ink-850/80 ring-1 ring-ink-700/50 px-2.5 py-2">
            <div className="flex items-center justify-between">
              <span className="text-[11px] text-ink-300 truncate">
                {s.displayName}
                {s.isBigBlind && <span className="text-gold-500 ml-1">BB</span>}
                {s.isSmallBlind && <span className="text-gold-500 ml-1">SB</span>}
              </span>
            ...
            <div className="flex gap-1 mt-1.5">
              {s.holeCards.length > 0 ? (
                ...
                <span className="text-[10px] text-ink-600">非公開</span>
              )}
            </div>
          </div>
        ))}
      ...
      <div>
        <h4 className="text-[11px] text-ink-500 mb-2">アクション履歴</h4>
          ...
          {hand.actions
            ...
            .map((a, i) => {
              ...
              return (
                <div key={i}>
                  {showStreetHeader && (
                    <div className="text-[10px] uppercase tracking-wider text-ink-600 mt-2 mb-1">
                  ...
                  <div className="flex items-center justify-between text-ink-300">
                    <span>
                      ...
                      {a.toAmount !== null && <span className="text-ink-100 tabular-nums"> → {a.toAmount.toLocaleString()}</span>}
                    </span>
                    <span className="text-ink-600 tabular-nums">pot {a.potBefore.toLocaleString()}</span>
                  </div>
                </div>
              );
            })}
