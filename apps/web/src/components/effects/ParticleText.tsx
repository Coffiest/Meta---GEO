...
/**
 * React Bits (https://reactbits.dev/text-animations/particle-text) の ParticleText を移植。
 ...
 *   (詳細は components/ui/Button.tsx のコメント参照)。呼び出し側は高さを持つ親要素で包む。
 * - キャンバスは常時 requestAnimationFrame を回し続けるため、ゲーム対戦画面など常時マウントされる
 *   場所には置かず、ログイン画面のように寿命が短い画面でのみ使うこと。
 */
...
const waitForFonts = async (font: string): Promise<void> => {
  ...
  try {
    ...
    /* フォント読み込み失敗時もフォールバックのfont-familyでサンプリングを続行する */
  }
