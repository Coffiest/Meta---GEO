...
裏面(伏せカード)だけは扱いが違います。**描画に使うのは `back_v3.png`** で、これは支給された
原画 `back_source.jpg` から `scripts/card-back.py` が書き出したものです(縮小・角の透過・減色のみ。
絵柄には手を入れません)。裏面を差し替えるときは、新しい原画を `back_source.jpg` として置いてから

```
python3 scripts/card-back.py
```

を実行してください。比率が変わった場合は、スクリプトが最後に出す指示どおり
`apps/web/src/components/PlayingCard.tsx` の `BACK_ASPECT` も合わせます。
(`back.png` は差し替え前の絵が残っているだけで、どこからも読まれていません。)
